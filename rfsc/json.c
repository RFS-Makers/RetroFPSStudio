    ...
    if (valueindex < 1 || valueindex > lua_gettop(l)) {
        if (error) {
            ...
            snprintf(buf, sizeof(buf) - 1,
                "internal error: invalid stack index: %d (top: %d)",
    ...
    if (lua_type(l, valueindex) == LUA_TSTRING) {
        ...
        while (lua_next(l, valueindex) != 0) {
            ...
            if (lua_rawlen(l, valueindex) > 0 && (
                ...
                return NULL;
            } else if (lua_rawlen(l, valueindex) > 0) {
                is_array = 1;
                lua_pop(l, 1);  // remove value only, keep key
                continue;
            }
            if (encoded == NULL) {
                encoded = (is_array ? strdup("[") : strdup("{"));
                if (!encoded) {
                    lua_pop(l, 2);  // remove iterated key + value
                    goto outofmemfail;
                }
            ...
            if (lua_type(l, -2) != LUA_TSTRING) {
                ...
                if (error) {
                    ...
                    snprintf(
                        ...
                        "have key of type %s", lua_typename(l, wrongtype)
                    );
                    *error = strdup(buf);
                }
                if (encoded)
